...
pub fn decrypt_file(
    ...
    // The encryptor writes a 27-byte header (magic + nonce), then `encrypt_next`
    // segments, then a final empty segment via `encrypt_last` (16-byte tag).
    // Every byte after the header except the last 16 belongs to `encrypt_next`
    // segments; the last 16 bytes are the final tag.
...
/// Rust type holding the unlocked vault key in memory. Managed via Tauri state.
...
/// Creates the vault with a passphrase and unlocks it in memory.
#[tauri::command]
pub async fn cmd_vault_setup(
    db: State<'_, Db>,
    vault_state: State<'_, VaultState>,
    passphrase: String,
) -> Result<bool, String> {
    let key = setup_vault(db.inner(), &passphrase)?;
...
pub async fn cmd_vault_unlock(
    ...
    let key = unlock_vault(db.inner(), &passphrase)?;
    *vault_state.key.lock().unwrap() = Some(key.0.to_vec());
    Ok(true)
}
...
#[tauri::command]
pub async fn cmd_vault_lock(vault_state: State<'_, VaultState>) -> Result<bool, String> {
