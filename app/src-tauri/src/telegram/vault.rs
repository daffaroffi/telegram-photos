...
//! the local database. The full `Peer` object is cached in memory so uploads
//! can target it without re-scanning dialogs.
...
/// Returns `(channel_id, access_hash, title)` for the vault, creating it if
/// needed, and caches the resolved `Peer` in the Telegram state.
...
/// Resolves the vault `Peer`: cached in-memory first, then a dialog scan.
pub async fn peer_from_vault(
    ...
    Err("Vault channel tidak ditemukan. Jalankan backup sekali untuk membuatnya.".into())
}
...
pub async fn cmd_get_or_create_vault(
    ...
    get_or_create_vault(&client, db.inner(), &state).await
}
